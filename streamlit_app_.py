...
# ---------------------------------------------------------------------------
# RNA_INTERFERENCE_TECHNICAL  (source: rna_interference_technical.html)
# ---------------------------------------------------------------------------
RNA_INTERFERENCE_TECHNICAL = '''
<h2 class="sr-only">Technical diagram comparing the siRNA and miRNA branches of RNA interference: Dicer and Argonaute machinery is shared, but siRNA pairs near-perfectly with its target and triggers slicing, while miRNA pairs only through its seed region in the 3'UTR and instead represses translation and triggers deadenylation.</h2>
<style>
  .stg { opacity: 0.12; transition: opacity .5s ease; }
  .stg.on { opacity: 1; }
  .pulse { animation: pulse 1.2s ease-in-out infinite; }
  @keyframes pulse { 0%,100%{opacity:1} 50%{opacity:.5} }
  #riscGroup { transition: transform 1.1s ease; }
  .dockedSi #riscGroup { transform: translateX(240px); }
  .dockedMi #riscGroup { transform: translateX(357px); }
  .rowbtns { display:flex; gap:8px; flex-wrap:wrap; margin-bottom:10px; }
  .rowbtns button.active { border-color: var(--border-accent); color: var(--text-accent); }
  .btnrow { display:flex; gap:10px; align-items:center; margin-top:12px; flex-wrap:wrap; }
  #stepLabel { font-size:13px; color:var(--text-secondary); }
...
<div class="rowbtns">
  <button id="btnSi" onclick="setPath('si')">siRNA pathway</button>
  <button id="btnMi" onclick="setPath('mi')">miRNA pathway</button>
</div>

<svg width="100%" viewBox="0 0 700 460" role="img">
<title>RNAi technical: siRNA vs miRNA branches sharing Dicer/Argonaute machinery</title>
<desc>The siRNA branch begins with long double-stranded RNA in the cytoplasm. The miRNA branch begins with a pri-miRNA hairpin transcribed in the nucleus, cropped by the Drosha/DGCR8 microprocessor into a pre-miRNA hairpin, and exported to the cytoplasm by Exportin-5. Both branches converge on Dicer, which produces a small RNA duplex, and Argonaute, which selects a guide strand and discards the other. Guide-loaded RISC then scans mRNAs: the siRNA guide finds a near-perfectly complementary site, usually in the coding sequence, and Argonaute's slicer activity cleaves the message. The miRNA guide instead pairs through its seed region alone with a site in the 3 prime untranslated region, an imperfect match that does not trigger slicing; instead it represses translation and leads to deadenylation and destabilization of the message.</desc>
<defs>
<marker id="arrow" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse"><path d="M2 1L8 5L2 9" fill="none" stroke="context-stroke" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></marker>
</defs>

<rect id="nucleusBox" class="stg" x="20" y="20" width="260" height="145" rx="10" fill="var(--surface-1)" stroke="var(--border-strong)" stroke-width="0.5"/>
<text class="ts" x="30" y="35">Nucleus</text>

<g id="priMirna" class="stg">
<path d="M75 115 L75 75" stroke="#378ADD" stroke-width="3" stroke-linecap="round"/>
<path d="M100 115 L100 75" stroke="#D85A30" stroke-width="3" stroke-linecap="round"/>
<path d="M75 75 Q87 55 100 75" stroke="var(--t)" stroke-width="2" fill="none"/>
<text class="ts" x="87" y="130" text-anchor="middle">pri-miRNA</text>
</g>

<g id="microprocessor" class="stg">
<circle cx="140" cy="70" r="18" class="c-amber"/>
<text class="ts" x="140" y="70" text-anchor="middle" dominant-baseline="central">DGCR8</text>
<text class="ts" x="140" y="98" text-anchor="middle">Drosha/DGCR8</text>
</g>

<g id="preMirnaNuc" class="stg">
<path d="M195 110 L195 85" stroke="#378ADD" stroke-width="3" stroke-linecap="round"/>
<path d="M215 110 L215 85" stroke="#D85A30" stroke-width="3" stroke-linecap="round"/>
<path d="M195 85 Q205 72 215 85" stroke="var(--t)" stroke-width="2" fill="none"/>
<text class="ts" x="205" y="125" text-anchor="middle">pre-miRNA</text>
</g>

<g id="exportin" class="stg">
<circle cx="205" cy="150" r="14" class="c-teal"/>
<text class="ts" x="205" y="150" text-anchor="middle" dominant-baseline="central">XPO5</text>
<line x1="205" y1="164" x2="205" y2="204" stroke="var(--t)" stroke-width="1.5" stroke-dasharray="3 2" marker-end="url(#arrow)"/>
</g>

<g id="dsRnaCyto" class="stg">
<path d="M55 210 L155 210" stroke="#378ADD" stroke-width="3" stroke-linecap="round"/>
<path d="M55 224 L155 224" stroke="#D85A30" stroke-width="3" stroke-linecap="round"/>
<text class="ts" x="105" y="242" text-anchor="middle">Long dsRNA (cytoplasm)</text>
</g>

<g id="dicer" class="stg">
<circle cx="230" cy="260" r="22" class="c-amber"/>
<text class="ts" x="230" y="260" text-anchor="middle" dominant-baseline="central">Dicer</text>
<text class="ts" x="230" y="290" text-anchor="middle">RNase III</text>
</g>

<g id="duplexProduct" class="stg">
<path d="M212 320 L248 320" stroke="#1D9E75" stroke-width="3" stroke-linecap="round"/>
<path d="M212 332 L248 332" stroke="#1D9E75" stroke-width="3" stroke-linecap="round" opacity="0.6"/>
<text class="ts" id="duplexLabel" x="230" y="348" text-anchor="middle">duplex</text>
</g>

<g id="passengerDiscard" class="stg">
<path d="M270 320 L290 336 M290 320 L270 336" stroke="#E24B4A" stroke-width="1.5" stroke-linecap="round"/>
<text class="ts" id="passengerLabel" x="280" y="352" text-anchor="middle">discarded</text>
</g>

<g id="riscGroup">
<ellipse cx="230" cy="380" rx="34" ry="26" class="c-purple"/>
<text class="th" x="230" y="380" text-anchor="middle" dominant-baseline="central">AGO2</text>
<path id="guideStrand" d="M264 380 L296 380" stroke="#D85A30" stroke-width="3" stroke-linecap="round" opacity="0"/>
</g>

<line id="targetLine" x1="380" y1="380" x2="620" y2="380" stroke="#1D9E75" stroke-width="3" stroke-linecap="round"/>
<circle cx="380" cy="380" r="6" fill="#B91C1C"/>
<text class="ts" x="470" y="368" text-anchor="middle">Coding sequence</text>
<rect x="560" y="370" width="55" height="20" fill="none" stroke="#EF9F27" stroke-width="1.5" stroke-dasharray="3 3"/>
<text class="ts" x="587" y="360" text-anchor="middle">3'UTR</text>
<g id="polyA">
<circle id="polyA1" cx="625" cy="380" r="3" fill="var(--t)" opacity="0.8"/>
<circle id="polyA2" cx="632" cy="380" r="3" fill="var(--t)" opacity="0.8"/>
<circle id="polyA3" cx="639" cy="380" r="3" fill="var(--t)" opacity="0.8"/>
<circle id="polyA4" cx="646" cy="380" r="3" fill="var(--t)" opacity="0.8"/>
<circle id="polyA5" cx="653" cy="380" r="3" fill="var(--t)" opacity="0.8"/>
<circle id="polyA6" cx="660" cy="380" r="3" fill="var(--t)" opacity="0.8"/>
</g>

<g id="cleavageMarks" class="stg">
<path d="M462 370 L478 390 M478 370 L462 390" stroke="#E24B4A" stroke-width="2" stroke-linecap="round"/>
</g>
<text class="th stg" id="outcomeTextSi" x="470" y="412" text-anchor="middle">AGO2 slices — mRNA degraded</text>

<g id="ribosomeBlock" class="stg">
<ellipse cx="580" cy="330" rx="24" ry="14" class="c-gray"/>
<ellipse cx="580" cy="346" rx="28" ry="10" class="c-gray"/>
<path d="M558 313 L602 359" stroke="#DC2626" stroke-width="3" stroke-linecap="round"/>
</g>
<text class="th stg" id="outcomeTextMi" x="580" y="412" text-anchor="middle">Translation repressed</text>
<text class="ts stg" id="deadenylationText" x="580" y="428" text-anchor="middle">poly-A tail shortened (deadenylation)</text>
</svg>
...
let path = 'si';
const labelsSi = [
  "Step 0 of 5 — long double-stranded RNA present in the cytoplasm",
  "Step 1 of 5 — Dicer (RNase III) cleaves it into ~21-nt siRNA duplexes with 2-nt 3\\u2032 overhangs",
  "Step 2 of 5 — the duplex loads into Argonaute2 (AGO2); the strand with the less stable 5\\u2032 end becomes the guide",
  "Step 3 of 5 — the passenger strand is cleaved and degraded, leaving a mature RISC\\u2013guide complex",
  "Step 4 of 5 — RISC scans mRNAs and finds a site with near-perfect complementarity",
  "Step 5 of 5 — AGO2's PIWI domain slices the target — the mRNA is degraded, the gene silenced"
];
const labelsMi = [
  "Step 0 of 6 — a pri-miRNA hairpin is transcribed in the nucleus",
  "Step 1 of 6 — the Drosha/DGCR8 microprocessor crops it into a pre-miRNA hairpin",
  "Step 2 of 6 — Exportin-5/RanGTP exports the pre-miRNA hairpin to the cytoplasm",
  "Step 3 of 6 — Dicer trims the terminal loop, producing a miRNA duplex",
  "Step 4 of 6 — the duplex loads into Argonaute; the guide strand is selected, miRNA* discarded",
  "Step 5 of 6 — RISC pairs with the target's 3'UTR through the seed region only — an imperfect match",
  "Step 6 of 6 — no slicing: translation is repressed and the mRNA is deadenylated and destabilized"
];
function maxStep() { return path === 'si' ? 5 : 6; }
function setPath(p) {
  path = p; step = 0;
  document.getElementById('btnSi').classList.toggle('active', p === 'si');
  document.getElementById('btnMi').classList.toggle('active', p === 'mi');
  render();
}
function render() {
  const labels = path === 'si' ? labelsSi : labelsMi;
  document.getElementById('stepLabel').textContent = labels[step];

  document.getElementById('nucleusBox').classList.toggle('on', path === 'mi');
  document.getElementById('priMirna').classList.toggle('on', path === 'mi');
  document.getElementById('microprocessor').classList.toggle('on', path === 'mi' && step >= 1);
  document.getElementById('microprocessor').classList.toggle('pulse', path === 'mi' && step === 1);
  document.getElementById('preMirnaNuc').classList.toggle('on', path === 'mi' && step >= 1);
  document.getElementById('exportin').classList.toggle('on', path === 'mi' && step >= 2);
  document.getElementById('dsRnaCyto').classList.toggle('on', path === 'si');

  const dicerOn = (path === 'si' && step >= 1) || (path === 'mi' && step >= 3);
  document.getElementById('dicer').classList.toggle('on', dicerOn);
  document.getElementById('dicer').classList.toggle('pulse', (path === 'si' && step === 1) || (path === 'mi' && step === 3));

  const duplexOn = (path === 'si' && step >= 2) || (path === 'mi' && step >= 4);
  document.getElementById('duplexProduct').classList.toggle('on', duplexOn);
  document.getElementById('duplexLabel').textContent = path === 'si' ? "siRNA duplex (2-nt 3' overhangs)" : 'miRNA duplex';

  const riscOn = (path === 'si' && step >= 2) || (path === 'mi' && step >= 4);
  document.getElementById('riscGroup').classList.toggle('on', riscOn);
  document.getElementById('guideStrand').setAttribute('opacity', riscOn ? '1' : '0');

  const discardOn = (path === 'si' && step >= 3) || (path === 'mi' && step >= 4);
  document.getElementById('passengerDiscard').classList.toggle('on', discardOn);
  document.getElementById('passengerLabel').textContent = path === 'si' ? 'passenger strand degraded' : 'miRNA* discarded';

  const svg = document.querySelector('svg');
  svg.classList.toggle('dockedSi', path === 'si' && step >= 4);
  svg.classList.toggle('dockedMi', path === 'mi' && step >= 5);

  document.getElementById('cleavageMarks').classList.toggle('on', path === 'si' && step >= 5);
  document.getElementById('outcomeTextSi').classList.toggle('on', path === 'si' && step >= 5);
  document.getElementById('targetLine').style.opacity = (path === 'si' && step >= 5) ? '0.25' : '1';

  const repressed = path === 'mi' && step >= 6;
  document.getElementById('ribosomeBlock').classList.toggle('on', repressed);
  document.getElementById('outcomeTextMi').classList.toggle('on', repressed);
  document.getElementById('deadenylationText').classList.toggle('on', repressed);
  ['polyA4', 'polyA5', 'polyA6'].forEach(id => {
    document.getElementById(id).setAttribute('opacity', repressed ? '0.12' : '0.8');
  });
}
function stepFwd() { if (step < maxStep()) step++; render(); }
function stepBack() { if (step > 0) step--; render(); }
...
REGISTRY = {
    ...
    "RNA interference": {
        ...
        "Technical": {
            ...
            "height": 560,
            "blurb": (
                "Branch between the siRNA and miRNA pathways: a nuclear "
                "Drosha/DGCR8 crop and Exportin-5 export feed the miRNA "
                "route into the same Dicer/Argonaute machinery as siRNA, "
                "but near-perfect pairing triggers slicing while seed-only "
                "3'UTR pairing instead represses translation and "
                "deadenylates the message."
            ),
        },
    },
    "Meiosis": {
